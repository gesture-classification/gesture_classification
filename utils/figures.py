...
def get_dataframe(Pilot_id: int, mounts: dict, data: str='X_train') -> pd.DataFrame:
    """ Функция создания датафрейма из данных словаря
    Аргументы:
    ---------
    Pilot_id (_int_)- номер пилота
...
def get_sensor_list(Pilot_id: int, mounts: dict, level_boundary: int, print_active=False) -> tuple[list, list]:
    ...
    print_active (bool)- печать активных жестов.
    """
...
def get_all_sensors_plot(Pilot_id: int, timesteps: list, mounts: dict, plot_counter='1'):
    ...
    fig.update_layout(
        title=dict(text=f'Fig.{plot_counter} - sensor signals of pilot #{Pilot_id}', x=.5, y=0.05, xanchor='center'),
...
def get_active_passive_sensors_plot(Pilot_id: int, timesteps: list, mounts: dict, level_boundary: int, plot_counter='1'):
    ...
    fig = make_subplots(rows=1, cols=2, 
                        subplot_titles=('active sensors', 'passive sensors')
    )
    ...
    fig.update_layout(title={'text':
        f'Fig.{plot_counter} - Active and passive sensors of pilot #{Pilot_id} within the period {timesteps[0],timesteps[1]}',
        'x': 0.5, 'y': 0.05}
    )
...
def plot_history(history, plot_counter: str):
    ...
    ax[1].set_title(f"F-1 score")
    ax[1].legend()

    fig.suptitle(f"Fig.{plot_counter} - Model learning", y=-0.1, fontsize=14)
              
    fig.savefig(f'../logs_and_figures/fig_{plot_counter}.png')
...
def get_gesture_prediction_plot(Pilot_id: int, i: int, y_pred_train_nn_mean: np.ndarray, mounts: dict, plot_counter: str):
    ...
    plt.suptitle(f'Fig.{plot_counter} - test #{i} of pilot {Pilot_id}', y=-0.01, fontsize=14)
    ...
    ax[0].set_title('OMG sensors signals')

    ax[1].imshow(y_train_nn[i].T, origin="lower")
    ax[1].set_aspect('auto')
    ax[1].set_title('Gesture class')
    ax[1].set_yticks(
    ...
    ax[2].set_title('Predicted probability of gesture class')
    ax[2].set_yticks(
        np.arange(5), ['Open', 'Pistol', 'Thumb', 'OK', 'Grab']
    )
...
def get_signal_and_train_plots(Pilot_id: int, timesteps: list, sensors: list, mounts: dict, plot_counter='1'):
    ...
    fig.update_layout(title={'text':
        f'Fig. {plot_counter} - Sensors #{sensors} signals of the pilot #{Pilot_id} and original manipulator command',
        'x': 0.5, 'y': 0.01}
    )
...
def get_signal_derivative_and_normalized_plot(Pilot_id: int, timesteps: list, sensors: list,
    ...
    fig.update_layout(title={'text':
        f'Fig. {plot_counter} - Sensor #{sensors} signal processing of the pilot #{Pilot_id}',
        'x': 0.5, 'y': 0.01}
    )

    fig.update_layout(width=1200, height=800, legend_title_text=f'Sensor number ',
                        xaxis_title_text='Period', yaxis_title_text='Sensor signal',  # yaxis_range=[0, 4000],
                        xaxis2_title_text='Period', yaxis2_title_text='Signal normalized',  # yaxis2_range=[0, 8],
...
def get_display_data(mounts: dict, plot_counter: str):
    ...
    # plt.show() #- не вызывать для корретного логгирования
    fig.suptitle(f"Fig.{plot_counter} - Sensor signals and gestures classes", y=-0.1, fontsize=12);
